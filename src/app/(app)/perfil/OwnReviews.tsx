...
export default function OwnReviews({ agentId, profile }: OwnReviewsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Review details */}
      <div className="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 space-y-6">
        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Opiniones de Colegas</h3>
            <p className="text-xs text-slate-400 mt-0.5">Lo que otros agentes inmobiliarios opinan al colaborar contigo</p>
          </div>
          
          <div className="flex items-center gap-1.5 bg-slate-900 px-3 py-1.5 rounded-xl text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center border-b border-slate-100 pb-6">
          <div className="md:col-span-4 text-center space-y-1">
            <h4 className="text-3xl font-black text-slate-800">{averageRating}</h4>
            <div className="flex justify-center text-slate-900 text-xs">
              {Array.from({ length: 5 }).map((_, i) => (
                <span 
                  key={i} 
                  className={`material-symbols-outlined text-base ${
                    i < Math.floor(parseFloat(averageRating)) ? 'text-slate-900 fill-current' : 'text-slate-200'
                  }`}
                >
                  star
                </span>
            ...
            {[5, 4, 3, 2, 1].map((stars) => {
              ...
              return (
                <div key={stars} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="material-symbols-outlined text-[9px] text-slate-900 fill-current">star</span>
                  ...
                  <span className="w-5 text-slate-400 text-right font-medium">{count}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
          {reviews.length === 0 ? (
            ...
            reviews.map((rev) => (
              <div key={rev.id} className="p-4 rounded-xl bg-slate-50/50 border border-slate-100/50 space-y-1.5">
                      ...
                      {Array.from({ length: 5 }).map((_, i) => (
                        <span 
                          ...
                          className={`material-symbols-outlined text-[10px] ${
                            i < rev.rating ? 'text-slate-900 fill-current' : 'text-slate-200'
                          }`}
                        >
                          star
                        </span>
